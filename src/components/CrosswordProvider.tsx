...
const initialDefault = { grid, crosswordWords, appState: {
    showClueList: false,
    selectedWord: null,
    selectedCell: null,
}};
...
export const CrosswordProvider = ({ children }: { children: React.ReactNode }) => {    
    const [initialState, setInitialState] = useState<{ state: CrosswordContext; isLoaded: boolean }>({
        state: initialDefault as CrosswordContext,
        isLoaded: false,
      });

    const [crossword, dispatch] = useReducer(crosswordReducer, initialState.state);
    
    useEffect(() => {
        setInitialState((prevState) => ({
          state: loadStateFromLocalStorage() || prevState.state,
          isLoaded: true,
        }));
      }, []);
    ...
    useEffect(() => {
        ...
        if(crossword && Object.keys(crossword).length > 0 && crossword?.grid != initialDefault?.grid) {
    ...
    return (
        <CrosswordContext.Provider value={crossword}>
            {initialState.isLoaded && initialState.state && Object.keys(initialState.state).length > 0 ?
